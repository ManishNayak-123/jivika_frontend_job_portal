...
function SkillsResultPage() {
  const { state } = useLocation();
  const score = state?.score || 0;
  const subject = state?.subject || "react";
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          <Link to= {`/services/skills/${subject}`} className="flex items-center justify-center gap-2 w-full py-4 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-100">
            <RefreshCcw size={18} /> Retake Assessment
          </Link>
          <Link to="/services/skills" className="flex items-center justify-center gap-2 w-full py-4 bg-white text-slate-600 border-2 border-slate-100 rounded-2xl font-bold hover:bg-slate-50 transition-all">
